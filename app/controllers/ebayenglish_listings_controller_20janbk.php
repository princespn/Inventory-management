...
class EbayenglishListingsController extends AppController {
	...
	{
		parent::beforeFilter();
		$this->Auth->allow(array('login','logout','index','edit','delete','import','menus'));
	}
		...
		function index () {
			...
			$string = explode(",",trim($this->data['EbayenglishListing']['all_item']));
				...
				if((!empty($prsku)) && (!empty($prname))){
					...
					$conditions = array('EbayenglishListing.product_code LIKE' => '%'.$prname.'%','EbayenglishListing.sku LIKE' => '%'.$prsku.'%');
				$this->paginate = array('limit' => 1000,'totallimit' => 2000,'order'=>'EbayenglishListing.id ASC','conditions' => $conditions);
					}
					if((!empty($prsku))){
					
					$conditions = array(
					'OR'=> array('EbayenglishListing.product_code LIKE' => "%$prsku%",'EbayenglishListing.sku LIKE' => "%$prsku%"));
					$this->paginate = array('limit' => 1000,'totallimit' => 2000,'order'=>'EbayenglishListing.id  ASC','conditions' => $conditions);
					}
	...
	function edit($id = null) {
		...
		if (!$id && empty($this->data)) {//print_r($id);die();
