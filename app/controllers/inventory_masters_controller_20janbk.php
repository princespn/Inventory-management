...
class InventoryMastersController extends AppController {
	...
	{
		parent::beforeFilter();
		$this->Auth->allow(array('login','logout','index','edit_inventory','import_inventory','delete_inventory','searchData'));
                                     ...
                                     public function searchData(){
                                                    ...
                                                    if((!empty($prsku)) && (!empty($prname))){  
                                                        $conditions = array('InventoryMaster.item_sku LIKE' => '%'.$prname.'%','InventoryMaster.item_sku LIKE' => '%'.$prsku.'%');
                                                          ...
                                                          $inventorymasters = $this->InventoryMaster->find('all',$conditions);
                                                          $this -> set(compact($inventorymasters));
                                                          $this -> set('_serialize',array('inventorymasters'));
